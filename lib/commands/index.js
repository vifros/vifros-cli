 ...
 *
 * @param {Array}   api_route              The route path to be tested.
 * @param {Object}  reference_in_apidoc    A reference to the segment in the docs of the last valid path.
 * @param {Array}   stack                  The last valid route path.
 *
 * @return {Object} An object with the properties:
 *                  {Boolean}   valid       true if the whole route path is valid, false otherwise.
 *                  {Object}    reference   A reference to the segment in the docs of the last valid path.
 *                  {Array}     stack       The last valid route path.
 */
// TODO: Add support for dynamic routes like :path.
function getAPIRouteObject(api_route, reference_in_apidoc, stack) {
  // Initialize seed variables, if is the case.
  stack = stack || [];
  reference_in_apidoc = reference_in_apidoc || apidoc;

  if (!api_route.length) {
    // If gets here then was the route was the top level one.
    return {
      valid    : false,
  ...
  if (apidoc.hasOwnProperty('children')
    ...
    // TODO: Add suppport when both `children` & `instances` are present in apidoc.
    ...
    // Neither of them are present, so the route is invalid.
    return {
      valid    : false,
  ...
  if (reference_in_apidoc[nested_attribute].hasOwnProperty(api_route[0])) {
    /*
    ...
    if (api_route.length > 1) {
      ...
      return getAPIRouteObject(
        ...
        reference_in_apidoc[nested_attribute][api_route[0]],  // The new valid reference to be saved.
     ...
     * so return this reference as the last one, and mark it as a valid route.
     */
    return {
      valid    : true,
      reference: reference_in_apidoc[nested_attribute][api_route[0]],
...
exports.init = function (cb) {
  ...
  // If the command doesn't need an apidoc route, then execute it immediately.
  ...
  /*
   * Checks if the api route exists.
   * If fully route is not valid, print usage information for the last valid path.
   */
