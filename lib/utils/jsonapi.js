...
/**
 * Prints to the console the errors returned by JSON+API, if any.
 *
 * @param {Object}    jsonapi_response
 */
exports.printErrorsFromResponse = function (jsonapi_response) {
  var Table = require('cli-table');

  var table = new Table({
    head : ['Code', 'Path', 'Title'],
    chars: {
  ...
  for (var i = 0, j = jsonapi_response.errors.length;
       i < j;
       i++) {

    table.push([
      jsonapi_response.errors[i].code || '',
      jsonapi_response.errors[i].path || '',
      jsonapi_response.errors[i].title || ''
    ]);
  }
